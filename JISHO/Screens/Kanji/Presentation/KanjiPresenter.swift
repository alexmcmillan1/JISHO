...
class KanjiPresenter: KanjiPresenting {
    ...
    private func readings(from responseItem: KanjiResponseItem) -> [KanjiReadingDisplayItem] {
        let kunReadings = KanjiReadingDisplayItem(type: .kun, values: responseItem.kun_readings.isEmpty ? "―" : responseItem.kun_readings.joined(separator: " · "))
        let onReadings = KanjiReadingDisplayItem(type: .on, values: responseItem.on_readings.isEmpty ? "―" : responseItem.on_readings.joined(separator: " · "))
        let nameReadings = KanjiReadingDisplayItem(type: .name, values: responseItem.name_readings.isEmpty ? "―" : responseItem.name_readings.joined(separator: " · "))
